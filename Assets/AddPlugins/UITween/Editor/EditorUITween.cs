...
public class EditorUITween : Editor
{
	...
	void SetStartValues ()
	{
        foreach(EasyTween temp in targets)
        {
            RectTransform selectedTransform = temp.rectTransform;

            if (temp.animationParts.PositionPropetiesAnim.IsPositionEnabled())
                selectedTransform.anchoredPosition = (Vector2)temp.animationParts.PositionPropetiesAnim.StartPos;

            if (temp.animationParts.ScalePropetiesAnim.IsScaleEnabled())
                selectedTransform.localScale = temp.animationParts.ScalePropetiesAnim.StartScale;

            if (temp.animationParts.RotationPropetiesAnim.IsRotationEnabled())
                selectedTransform.localEulerAngles = temp.animationParts.RotationPropetiesAnim.StartRot;

            if (temp.animationParts.FadePropetiesAnim.IsFadeEnabled())
            {
                if (temp.IsObjectOpened())
                    SetAlphaValue(selectedTransform.transform, temp.animationParts.FadePropetiesAnim.GetEndFadeValue());
                else
                    SetAlphaValue(selectedTransform.transform, temp.animationParts.FadePropetiesAnim.GetStartFadeValue());
            }
        }
	}
	...
	void SetEndValues ()
	{
        ...
        foreach (EasyTween temp in targets)
        {
            ...
            if (temp.animationParts.PositionPropetiesAnim.IsPositionEnabled())
                selectedTransform.anchoredPosition = (Vector2)temp.animationParts.PositionPropetiesAnim.EndPos;

            if (temp.animationParts.ScalePropetiesAnim.IsScaleEnabled())
                selectedTransform.localScale = temp.animationParts.ScalePropetiesAnim.EndScale;

            if (temp.animationParts.RotationPropetiesAnim.IsRotationEnabled())
                selectedTransform.localEulerAngles = temp.animationParts.RotationPropetiesAnim.EndRot;
